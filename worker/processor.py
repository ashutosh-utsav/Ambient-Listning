...
class SessionProcessor:
    ...
    async def _trigger_transcription_task(self):
        if self.transcription_buffer:
            ...
            try:
                ...
                wav_blob_name = f"{self.audio_base_path}/{wav_file_name}"
                ...
                await self.error_reporter(self.session_id, "BLOB_UPLOAD_FAILED", str(e))
            finally:
                del wav_bytes
            
            in_memory_wav.seek(0)
            in_memory_wav.name = f"chunk_{self.chunk_index}.wav"
            task = asyncio.create_task(_transcribe_chunk(in_memory_wav, self.chunk_index, self.session_id, self.error_reporter))
    ...
    async def finalize_session(self):
        ...
        for index, transcript_chunk in partial_transcripts:
            ...
            chunk_text = transcript_chunk["text"] if isinstance(transcript_chunk, dict) else getattr(transcript_chunk, "text", "")
            text_parts.append(chunk_text)

            segments = transcript_chunk.get("segments", []) if isinstance(transcript_chunk, dict) else getattr(transcript_chunk, "segments", []) or []
            if segments:
                ...
                last_segment_end = segments[-1].get("end", 0) if isinstance(segments[-1], dict) else getattr(segments[-1], "end", 0)
        ...
        try:
            ...
            await container_client.upload_blob(
                name=self.transcript_blob_name,
        ...
        while attempt < retries:
            try:
                ...
                if attempt < retries:
                    ...
                    await error_reporter(session_id, "TRANSCRIPTION_FAILED", f"Chunk {chunk_index+1} failed all retries: {e}")
